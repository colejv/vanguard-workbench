...
def build_tasks(out_dir: str, resume_context: dict = None) -> dict:
    ...
    t_stage2 = Task(
        description=(
            ...
            "with two real structured list arguments — nodes and edges — passed directly as the "
            "tool's arguments, never assembled into a JSON string yourself. Annex B reads this "
            "file; without it the KCAG cannot be built. Rules:\n"
            "  - `nodes` argument: a list of objects. Every node has: id, node_type (one of: "
            "privilege, technique, property, countermeasure, goal), criticality (1-10; "
            "CDL/fires/OT = 10).\n"
            "  - `edges` argument: a list of objects. Every edge has: source, target, technique "
            "(the grounded ID), difficulty (LOW|MEDIUM|HIGH), effect (DECEIVE|DISRUPT|DEGRADE|"
            "DESTROY or null), vec (V-NN).\n"
    ...
    t_annexC = Task(
        description=(
            _annexC_resume_prefix +
            "Annex C: construct the evidence-driven BBN, ingest the Annex B KCAG heuristic factor, "
