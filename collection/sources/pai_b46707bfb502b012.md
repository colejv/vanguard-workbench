# ngc-examples/ncsp/README.md at master · NVIDIA/ngc-examples

<!-- source: https://github.com/NVIDIA/ngc-examples/blob/master/ncsp/README.md -->
...
- Easy access to the CSP to create and run VMS
- Understanding what the CSP is capable of and how to control it
- Explore differences between the commands, features and performance of the different CSPs
- Provide a simple command line and scriptable interface to create and control VMs independent of the CSP.
...
- Code is simple well documented python, easy to look at and tweak
- Provides a identical CLI interface regardless of the CSP. Users can easily start a VM on a CSP without knowing any of the internal features of the CSP
- CSP specific code and error checking is pretty well isolated to specific CSP dependent modules. All the command parsing, error handling, persistence and reports are independent of that code.
- CSP dependent parameters, like the image name, region or login name are defaulted in a CSP dependent manor and for the most part are not needed to be tweaked by the user. Yet all these parameters can be tweaked on the command line
- These tweakable configuration parameters are persistently stored in a file, and remain set until they are changed or cleared in mass.
- The CSP returned object values, such as VM ids and IP address are also stored in that same persistent file, and can be viewed and tweaked in the same way as other parameters. You don’t need to remember them, and the command make use if them when needed
- Easy to add a interface to a new CSP
...
ncsp gcp ssh uptime
ncsp gcp deleteVMThere might be a few CSP specific changes that you have to account for -- like the number of GPUs as '--accelerator_count=8' in the Google example above.
...
└── aws
    ...
    └── logs
        ├── cmds
        └── test
The data/args file contains all the command line option falues and response that are currently active. Deleting the VM or the clean command deletes that file, restoring all options back to programmed defaults.
...
The CSP dependent code is loosely placed in the funcs.py file for each csp. These are read in when the ncsp application is started, based on the 2nd argument. Thus to support any CSP, all that's necessary is having a file by the proper name in the directory.
